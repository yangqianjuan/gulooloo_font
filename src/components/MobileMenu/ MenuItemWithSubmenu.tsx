...
const MenuItemWithSubmenu: React.FC<Props> = ({
  ...
  return (
    <div>
      ...
      {isOpen && (
        <ul className="ml-4 mt-2  text-base text-gray-700 mb-[32px]">
          {item.children?.map((child) => (
            <li
              key={child.key}
              className="sm:text-[rgba(4,30,84,1)] text-[rgba(4,30,84,0.32)] hover:text-[rgba(4,30,84,1)] 2xl:text-[20px] sm:text-[18px] text-[16px] mt-[24px]"
            >
              {item.isLang ? (
