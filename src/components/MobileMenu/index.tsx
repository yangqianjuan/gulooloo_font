...
const MobileMenu: React.FC<MobileMenuProps> = ({
  ...
  return (
    ...
    >
      <div className="flex justify-end px-[24px] py-[20px]">
        <button onClick={onClose} aria-label="closed modal">
          <X size={24} />
        </button>
      </div>

      <nav className="pt-[20px] px-[24px] text-[16px]">
        <ul>
          {items.map((item) => (
            <li
              key={item.key}
              className="py-[16px] border-b-[1px] border-[rgba(4,30,84,0.08)]  text-[rgba(4,30,84,0.48)] hover:text-[rgba(4,30,84,1)]"
            >
              {item.children ? (
                <MenuItemWithSubmenu
                  item={item}
